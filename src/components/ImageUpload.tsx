...
const ImageUpload: React.FC<ImageUploadProps> = ({ title, image, onImageChange, disabled = false }) => {
  ...
  return (
    ...
    `}>
      <div className={`flex items-center justify-between mb-4 ${isDark ? 'text-gray-200' : 'text-gray-800'}`}>
      ...
      {!image ? (
          ...
          className={`
            ...
            ${isDragActive
              ...
              : `
                  ...
                  ${isDark ? 'dark:border-gray-600 dark:hover:border-indigo-500 dark:hover:bg-gray-700' : ''}
                `
            }
        ...
        >
          ...
          <p className={`text-xs ${isDark ? 'text-gray-500' : 'text-gray-400'}`}>Supports JPEG, PNG, WebP (max 10MB)</p>
        </div>
      ) : (
