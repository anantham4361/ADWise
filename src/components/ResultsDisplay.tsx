...
import {
  Trophy, Target, TrendingUp, Image, Video, FileText
} from 'lucide-react';
...
const ResultsDisplay: React.FC<ResultsDisplayProps> = ({ result }) => {
  const { ad_a_scores, ad_b_scores, winner, explanation, ad_type } = result;

  // Get the criteria keys excluding "total"
  const criteriaKeys = Object.keys(ad_a_scores).filter(key => key !== 'total');

  // Prepare data for bar and radar charts
  const barChartData = criteriaKeys.map((key) => ({
    criterion: key,
    'Ad A': ad_a_scores[key as keyof typeof ad_a_scores],
    'Ad B': ad_b_scores[key as keyof typeof ad_b_scores],
  }));

  const radarData = barChartData;
